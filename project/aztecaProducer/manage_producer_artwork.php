...
// Each role will have different permissions and limited access as defined
  ...
  $q = "SELECT a_id,art_title,artist_id,art_status,image,DATE_FORMAT(created_on,'%M %d, %Y') AS artwork_created_date FROM azteca_artwork where producer_name=:p_id";    
 $query=$dbh->prepare($q);
                  ...
                  <tr bgcolor="' . $bg . '">
                     ...
                     echo '<td>' . $row['artwork_created_date'] . '</td>
                      <td>' . status_toString($row['art_status']) . '<div class="progress">
                          ...
                          <td align="center"><a href="process_image.php?a_id=' . $row['a_id'] . '"><span class="glyphicon glyphicon-picture"></span></a></td>
                    ...
                    if($row['art_status'] !=20){
                          echo '<td align="center"><a href="to_publish_artwork.php?a_id=' . $row['a_id'] . '"><span class="glyphicon glyphicon-check"></span></a></td>';               
                    } else{

                        echo "<td align=center> It's Published</td>";
                      }   
                  echo '</tr>
              </tbody>';
      } // End of WHILE loop.
